...
def build_system_prompt(personality_summary: str) -> str:
    ...
    rules = (
        ...
        "7. ТИП ОТВЕТА: Ты можешь выбрать, отправить текстовый ответ, голосовой или оба сразу. Допустимые значения для поля \"reply_type\": \"text\", \"voice\", \"both\".\n"
        ...
        "   - Для сообщений доступно поле \"reply_type\": \"text\" (только текст), \"voice\" (только ГС) или \"both\" (текст и ГС). Ты можешь комбинировать их в любом порядке (например, сначала прислать ГС через 1 сек, а затем текстовое пояснение через 3 сек).\n"
        ...
        "      \"reply_type\": \"text\" | \"voice\" | \"both\" // только для type=\"message\": тип ответа для этого конкретного сообщения (текст, голосовое или оба)\n"
        "    }\n"
        "  ],\n"
        ...
        "  \"explanation\": \"подробное объяснение твоего решения на русском языке (мысли о стиле чата, почему ты решил ответить или промолчать, почему выбрал именно этот смайлик, эффект или режим голоса)\"\n"
        "}"
    )
    return rules
